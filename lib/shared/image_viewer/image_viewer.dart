      ...
      child: Stack(
        children: [
          if (widget.image.runtimeType != String ||
              (widget.image.runtimeType == String && download))
            ClipRRect(
              borderRadius: BorderRadius.circular(10),
              child: InkWell(
                onTap: () {
                ...
                child: PhotoView(
                  ...
                  imageProvider:
                      DefaultInteractiveImage.imageProvider(widget.image),
          ...
          if (widget.image.runtimeType == String && download == false)
            DownloadFiles(url: widget.image, folder: 'Yemen Net')
        ],
      ),
    );
  }
...
class DefaultInteractiveImage extends StatefulWidget {
  const DefaultInteractiveImage(
      {Key? key, required this.images, required int selectedIndex})
      : super(key: key);
  final List<dynamic> images;
                            ...
                            .map(
                              (image) => Stack(
                                children: [
                                  PhotoView(
                                    imageProvider:
                                        DefaultInteractiveImage.imageProvider(
                                            image),
                                    initialScale:
